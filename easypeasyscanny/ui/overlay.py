# Extracted overlay/OSD utilities from ui/webapp.py
from typing import Any, Dict, Optional

# === Migrated Overlay Blocks ===
class MiningAPI:
    def __init__(self):
        # Konfiguration
    ...
    def build_rock_database(self, system):
        ...
        for rock_type, rock_data in system_data.items():
            if rock_type in rock_properties:
                ...
                # Extrahiere Stats direkt aus JSON ohne Berechnung
                stats = {
                    'cluster_min': rock_data.get('clusterCount', {}).get('min', 1),
                    'cluster_max': rock_data.get('clusterCount', {}).get('max', 11),
                    'cluster_med': rock_data.get('clusterCount', {}).get('med', 6),
                    'mass_min': rock_data.get('mass', {}).get('min', 0),
                    'mass_max': rock_data.get('mass', {}).get('max', 182000),
                    'mass_med': rock_data.get('mass', {}).get('med', 8900),
                    'instability_min': rock_data.get('inst', {}).get('min', 0),
                    'instability_max': rock_data.get('inst', {}).get('max', 711),
                    'instability_med': rock_data.get('inst', {}).get('med', 46),
                    'resistance_min': rock_data.get('res', {}).get('min', 0),
                    'resistance_max': rock_data.get('res', {}).get('max', 0.64),
                    'resistance_med': rock_data.get('res', {}).get('med', 0.16)
                }
    ...
    def calculate_rock_stats(self, rock, signal):
        """Berechne Stats für Rock - Verwende direkt die JSON-Daten"""
        multima = rock.get('multima_factor', 1)
        rock_type = rock.get('rock_type', '')

        # Hole die Daten direkt aus rocks_data
        if self.current_system in self.rocks_data and rock_type in self.rocks_data[self.current_system]:
            rock_data = self.rocks_data[self.current_system][rock_type]

            # Cluster Stats aus JSON
            cluster_min = rock_data.get('clusterCount', {}).get('min', 1)
            cluster_max = rock_data.get('clusterCount', {}).get('max', 11)
            cluster_med = rock_data.get('clusterCount', {}).get('med', 6)

            # Mass Stats aus JSON (bereits in kg, konvertiere zu Tonnen für Anzeige)
            mass_min = rock_data.get('mass', {}).get('min', 0)
            mass_max = rock_data.get('mass', {}).get('max', 182000)
            mass_med = rock_data.get('mass', {}).get('med', 8900)

            # Konvertiere zu Tonnen (durch 1000) und formatiere für Anzeige
            rock_mass_min = mass_min / 1000
            rock_mass_max = mass_max / 1000
            rock_mass_med = mass_med / 1000

            # Instability Stats aus JSON
            instability_min = rock_data.get('inst', {}).get('min', 0)
            instability_max = int(rock_data.get('inst', {}).get('max', 711))
            instability_med = int(rock_data.get('inst', {}).get('med', 46))

            # Resistance Stats aus JSON (von 0-1 zu 0-100%)
            resistance_min = int(rock_data.get('res', {}).get('min', 0) * 100)
            resistance_max = int(rock_data.get('res', {}).get('max', 0.64) * 100)
            resistance_med = int(rock_data.get('res', {}).get('med', 0.16) * 100)

            # Bei Multima: multipliziere entsprechende Werte
            if multima > 1:
                cluster_max = min(cluster_max * multima, 30)
                cluster_med = min(cluster_med * multima, 20)
                rock_mass_max = rock_mass_max * multima
                rock_mass_med = rock_mass_med * multima
            ...
            return {
                ...
                'mass': {
                    'min': f"{rock_mass_min:.0f}",
                    'max': f"{rock_mass_max:.0f}k",
                    'med': f"{rock_mass_med:.1f}k"
                },
                'instability': {
                    'min': instability_min,
                    'max': instability_max,
                ...
                'resistance': {
                    'min': f"{resistance_min}%",
                    'max': f"{resistance_max}%",
                    'med': f"{resistance_med}%"
                }
            }

        # Fallback falls keine Daten gefunden
        return {
            'cluster': {'min': 1, 'max': 11, 'med': 6},
            'mass': {'min': '0', 'max': '182k', 'med': '8.9k'},
            'instability': {'min': 0, 'max': 500, 'med': 250},
            'resistance': {'min': '0%', 'max': '50%', 'med': '25%'}
        }
...
'''
        ...
        for mineral_name, percentage, color in minerals:
            html += f'''
                <tr>
                    <td class="mineral-bar-cell">
                        <div class="mineral-bar">
                            <div class="mineral-fill" style="background: {color}; width: {percentage}%;"></div>
                            <span class="mineral-name-in-bar">{mineral_name}</span>
                        </div>
                    </td>
                    <td class="mineral-percentage-cell">
                        <span class="mineral-percentage">{percentage}%</span>
                    </td>
                </tr>
'''
